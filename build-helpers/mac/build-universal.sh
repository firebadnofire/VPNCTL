...
ensure_rust_targets() {
  if ! command -v rustup >/dev/null 2>&1; then
    echo "error: rustup is required to install macOS universal Rust targets." >&2
    exit 1
  fi
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "error: the macOS universal helper must run on macOS" >&2
  exit 1
fi
...
for command in cargo hdiutil lipo node pnpm rustc rustup; do
...
bundle_root="${repo_root}/target/universal-apple-darwin/release/bundle"
echo "Universal app: ${bundle_root}/macos/VPN Appliance Manager.app"

dmg_files=("${bundle_root}/dmg/"*.dmg)
if [[ -e "${dmg_files[0]}" ]]; then
  for dmg_file in "${dmg_files[@]}"; do
    echo "Universal DMG: ${dmg_file}"
  done
else
  echo "Universal DMG directory: ${bundle_root}/dmg"
fi
