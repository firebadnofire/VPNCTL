...
pub fn render_corefile(zone: &str) -> Result<String, DnsError> {
    let zone = normalize_zone(zone)?;
    Ok(format!(
        "{zone} {{\n    errors\n    auto {{\n        directory /etc/coredns/zones db.(.*) {{1}}\n        reload 5s\n    }}\n}}\n\n. {{\n    errors\n    health\n    ready\n    hosts /etc/coredns/hosts/blocklist.hosts {{\n        ttl 60\n        reload 30s\n        fallthrough\n    }}\n    forward . tls://1.1.1.1 tls://1.0.0.1 {{\n        tls_servername one.one.one.one\n        health_check 5s\n        policy round_robin\n    }}\n    cache 60\n}}\n"
    ))
}

pub fn parse_hostslist_domains(contents: &str) -> Vec<String> {
    ...
    for line in contents.lines() {
        ...
        let candidates = if fields.first().is_some_and(|field| {
            field.parse::<Ipv4Addr>().is_ok() || field.parse::<Ipv6Addr>().is_ok()
        }) {
            &fields[1..]
        } else {
            &fields[..]
        ...
        for candidate in candidates {
            if let Some(domain) = normalize_blocklist_domain(candidate) {
...
pub fn render_blocklist_hosts(domains: &[String]) -> String {
    let mut normalized = BTreeSet::new();
    for domain in domains {
        if let Some(domain) = normalize_blocklist_domain(domain) {
            normalized.insert(domain);
        }
    }
    let mut output =
        "# Generated by VPN Appliance Manager. Selected hostlists return invalid addresses.\n"
            .to_owned();
    for domain in normalized {
        writeln!(output, "0.0.0.0 {domain}").expect("writing to a String cannot fail");
        writeln!(output, ":: {domain}").expect("writing to a String cannot fail");
    }
    output
}
...
fn normalize_blocklist_domain(value: &str) -> Option<String> {
...
mod tests {
    ...
    #[test]
    fn corefile_loads_hosts_blocklist_before_public_forwarding() {
        let corefile = render_corefile("internal").unwrap();
        assert!(corefile.contains("hosts /etc/coredns/hosts/blocklist.hosts"));
        assert!(
            corefile
                .find("hosts /etc/coredns/hosts/blocklist.hosts")
                .unwrap()
                < corefile.find("forward . tls://1.1.1.1").unwrap()
        );
    }

    #[test]
    fn parses_and_renders_blocklist_hosts_with_invalid_addresses() {
        let domains = parse_hostslist_domains(
            "0.0.0.0 Ads.Google.Com # ad host\n127.0.0.1 localhost\nmalware.test\n",
        );
        let hosts = render_blocklist_hosts(&domains);
        assert!(hosts.contains("0.0.0.0 ads.google.com\n"));
        assert!(hosts.contains(":: ads.google.com\n"));
        assert!(hosts.contains("0.0.0.0 malware.test\n"));
